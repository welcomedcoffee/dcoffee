            ...
            <div class="mt_rli">
                ...
                <div id="register">
                    <?php if($type == 'pwd') { ?>
                        ...
                        <input type="hidden" name="mer_id" value="<?= Html::encode($merchant->mer_id) ?>">
                        ...
                        <input type="hidden" id="phone_id" name="stu_id" value="<?= Html::encode($merchant->mer_id) ?>">
                        ...
                        <input type="hidden" id="id" name="stu_id" value="<?= Html::encode($merchant->mer_id) ?>">
                        <input type="hidden" name="type" value="pay">
                            <span style="font-size: 12px;margin-left:5px">
                                <span style="color: #da1f29">*</span>
                                <span style="display:inline-block;">您的手机号码</span>&nbsp;:
                                <input name="str_phone" id="str_phone" class="in1" placeholder="请输入手机号码" style="height:25px" type="text" value="<?= Html::encode($merchant->mer_phone) ?>">
                            </span>
                            <br>
...
<script type="text/javascript">
    ...
    $(document).on('blur', '#phones', function(){
        ...
        if (!reg.test(phone)){
            ...
            $.ajax({
               type: "POST",
               url: "<?= Url::to(['consumption/userphone'])?>",
               data: "phone="+phone,
               success: function(msg){
                 if (msg == 2) {
    ...
    $(".phone_code").blur(function(){
        var phone = "<?= Html::encode($merchant->mer_phone) ?>";
        var code = $(this).val()
        $.ajax({
               type: "POST",
               url: "<?= Url::to(['consumption/validationcodes'])?>",
    ...
    $("#oldpassword").blur(function(){
        ...
        $.ajax({
           type: "POST",
           url: "<?= Url::to(['consumption/userpwd'])?>",
